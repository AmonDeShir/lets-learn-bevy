...
use bevy::{prelude::{Component, Transform, Plugin, Query, Res, Vec3, Quat}, time::Time};
...
#[derive(Component)]
pub struct RotationAnimator(pub LinkedList<AnimationState<Quat, Quat>>);
...
pub fn animate_rotation(
  time: Res<Time>,
  mut query: Query<(&mut RotationAnimator, &mut Transform)>
) {
  for (mut animation_list, mut transform) in query.iter_mut() {
    let animation = match animation_list.0.front_mut() {
      Some(data) => data,
    ...
    match animation {
      AnimationState::Request(rotation, time) => {
        let speed = (*rotation - transform.rotation) / *time;
        *animation = AnimationState::Animate(*rotation, *time, speed);
      },
      AnimationState::Animate(rotation, remaining_time, speed) => {
        if *remaining_time <= 0.0 {
          animation_list.0.pop_front();
        ...
        if *remaining_time - time.delta_seconds() <= 0.0 {
          ...
          transform.rotation.x += speed.x * time.delta_seconds();
          transform.rotation.y += speed.y * time.delta_seconds();
          transform.rotation.z += speed.z * time.delta_seconds();
          transform.rotation.w += speed.w * time.delta_seconds();
        }
        
        *animation = AnimationState::Animate(*rotation, *remaining_time - time.delta_seconds(), *speed);
      }
    }
  }
}